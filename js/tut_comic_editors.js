...
const BUBBLE_SHAPES = ["椭圆对白框", "圆角矩形", "云朵思考框", "爆炸喊话框", "方形旁白框", "无边框文字"];

const clamp = (value, low = 0, high = 1) => Math.max(low, Math.min(high, Number(value) || 0));
const clone = (value) => JSON.parse(JSON.stringify(value));

function hideStore(store) {
    store.type = "converted-widget";
...
function parseBubbles(value) {
    try {
        const parsed = JSON.parse(value);
        if (parsed?.version !== 1 || !Array.isArray(parsed.bubbles)) return { version: 1, bubbles: [] };
        ...
        return { version: 1, bubbles };
    } catch { return { version: 1, bubbles: [] }; }
}
...
function ensureBubbleStyle() {
    ...
    style.textContent = `
      .tut-bubble-wrap{display:flex;flex-direction:column;gap:8px;height:100%;min-height:0;color:#ddd;font:12px sans-serif;overflow:hidden}
...
function drawBubblePath(context, bubble, x, y, w, h) {
    ...
    if (bubble.shape === "椭圆对白框") context.ellipse(x, y, w / 2, h / 2, 0, 0, Math.PI * 2);
        ...
        for (let point = 0; point < 32; point++) {
            const angle = -Math.PI / 2 + point * Math.PI / 16, radius = point % 2 ? .39 : .5;
        ...
        for (let point = 0; point < 12; point++) {
            const angle = point * Math.PI / 6, px = x + Math.cos(angle) * w * .42, py = y + Math.sin(angle) * h * .38;
            const radius = Math.max(4, Math.min(w, h) / 6);
...
function drawBubbleText(context, bubble, x, y, w, h) {
    const fontSize = Math.max(9, Math.min(72, Number(bubble.font_size) || 36));
    context.fillStyle = bubble.text_color || "#111111"; context.font = `${fontSize}px sans-serif`;
    context.textAlign = "center"; context.textBaseline = "middle";
    const maxWidth = Math.max(20, w * .78), lineHeight = fontSize * 1.18, lines = [];
    ...
    const visible = lines.slice(0, Math.max(1, Math.floor(h * .76 / lineHeight)));
...
function installBubbleEditor(node) {
    ...
    hideStore(store);
    const previousStoreCallback = store.callback;
    let data = parseBubbles(store.value), selected = -1, dragState = null, sourcePreview = null;
    ...
    const font = document.createElement("select"); font.className = "tut-bubble-select"; (fontWidget.options?.values || [fontWidget.value]).forEach((value) => font.add(new Option(value, value)));
    ...
    panels.content.append(
        selectedLabel, bubbleField("台词", text), bubbleField("对话框形状", shape), bubbleField("字体", font),
    ...
    panels.style.append(
        ...
        bubbleField("整体透明度", opacityRow), bubbleField("边框宽度", borderRow), colorFields,
    );
    ...
    function drawBubble(bubble, index) {
        const x = bubble.x * canvas.width, y = bubble.y * canvas.height;
        const w = bubble.w * canvas.width, h = bubble.h * canvas.height;
        context.save(); context.globalAlpha = clamp(bubble.opacity);
        context.fillStyle = bubble.fill_color || "#fff"; context.strokeStyle = bubble.border_color || "#111"; context.lineWidth = Math.max(1, bubble.border_width || 1);
        drawBubblePath(context, bubble, x, y, w, h);
        if (bubble.shape !== "无边框文字") { context.fill(); context.stroke(); }
        drawBubbleText(context, bubble, x, y, w, h);
        context.restore();
        if (index === selected) {
    ...
    function resizeCanvasToPreview() {
        ...
        canvas.width = Math.max(160, Math.round(naturalWidth * scale)); canvas.height = Math.max(120, Math.round(naturalHeight * scale));
    ...
    function render() {
        ...
        [text, shape, font, fontSize, opacity, borderWidth, textColor, fillColor, borderColor, remove, toTop, moveUp, moveDown, toBottom].forEach((control) => control.disabled = disabled);
        count.textContent = `${data.bubbles.length} / 32 个对话框`;
        selectedLabel.textContent = bubble ? `已选择：对话框 ${selected + 1}` : "未选择对话框";
        if (bubble) {
            text.value = bubble.text || ""; shape.value = bubble.shape; font.value = bubble.font_name || fontWidget.value;
        ...
        } else { text.value = ""; fontSizeValue.textContent = "—"; opacityValue.textContent = "—"; borderValue.textContent = "—"; }
        renderLayerList();
    }
    ...
    add.addEventListener("click", () => { if (data.bubbles.length >= 32) return; data.bubbles.push(newBubble(fontWidget.value, data.bubbles.length)); selected = data.bubbles.length - 1; save(); render(); });
    ...
    widget.computeSize = (width) => [width, 820];
    node.__tutComicBubbleEditor = true;
    const ensureDefaultSize = () => node.setSize?.([Math.max(node.size?.[0] || 0, 1100), Math.max(node.size?.[1] || 0, 960)]);
